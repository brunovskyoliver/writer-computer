...
- Vim mode: [`SPECs/vim-mode/spec.md`](SPECs/vim-mode/spec.md) + [`plan.md`](SPECs/vim-mode/plan.md) + [`tasks.md`](SPECs/vim-mode/tasks.md) — opt-in modal Vim editing via `@replit/codemirror-vim`: `editor.vim-mode` setting and palette toggle, footer mode indicator and `:` prompt host, `:w`/`:q`/`:q!`/`:wq`/`:x` through the app's own save and tab-close paths, clipboard bridge for the unnamed register.

- Tab tiling and split panes: [`SPECs/tab-tiling-splits/spec.md`](SPECs/tab-tiling-splits/spec.md) + [`plan.md`](SPECs/tab-tiling-splits/plan.md) + [`tasks.md`](SPECs/tab-tiling-splits/tasks.md) — Spec Kit planning and task breakdown complete. **Phases 1 and 2 (setup + foundation, T001-T014) are done** except T013: the layout tree, pane-derived active selectors, tab-keyed editor registrations and view state, the shared Markdown buffer across duplicate views, one drawing session per path, and stable tab-body hosts all landed, and the app still renders as a single pane. T013 is blocked on missing component-test infrastructure (node-only test runner, no jsdom) and the stable-host rendering has not been runtime-verified yet. **Phase 3 (US1, sidebar split MVP, T015-T019) is done**: one window-local pointer coordinator owns threshold, capture, per-frame hit-testing, and every cancel path; the sidebar tree is an adapter on it; a file selection dropped on a pane edge splits it and on the centre opens there, loaded all-or-nothing and revalidated against layout revision and workspace before one atomic commit. Not yet runtime-verified, and there is no drop preview until Phase 5. Phase 4 (US2, tab moves) is next. Tile a window's editor area into a tree of panes, each with its own tab strip; drag a file from the sidebar's Everything tree or a tab from any strip onto a pane edge to split it vertically or horizontally, onto its centre to open it there, with a translucent orange overlay previewing the target region during the drag. Shared document buffers across panes, per-tab history that travels with the tab, resizable dividers, layout persisted in the session with old flat-tab sessions migrating to one pane. Two existing gestures are resolved rather than broken: sidebar drag keeps meaning move-on-disk when released inside the tree, and the tab strip stops dragging the OS window while a tab drag is live. Tear-off to a new OS window stays out of scope.

- Drawing performance and explicit saves: [`SPECs/Agent/worksheet-drawing-performance.md`](SPECs/Agent/worksheet-drawing-performance.md) — remove per-stroke background exports; save on Cmd+S, tab close and app quit.
