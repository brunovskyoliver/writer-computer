...
- [x] T022 **Done at the head of Phase 5** — `locationBehavior` throws on an unregistered kind (`page-kinds/index.ts`), and it is reached from `deriveActiveFilePath` and `serializeLocation` on every tab activation and session save. Returning `{ kind: "drawing" }` before T025/T026/T032 exist crashes the data layer, not the view. It also needs the load-path guard: the six sites pair location construction with `ensureFileLoaded`, which would read the SVG as markdown into `openFiles` with the save machinery attached — the same overwrite T031 guards against. Third mechanism in the same class: several sites set `activeFilePath: path` directly instead of deriving it through `locationPrimaryPath`, so a drawing would publish as the active _file_ path and anything keyed off it (editor mount, save scheduling, statusbar) would treat it as an open note. Dispatch, kind, view, and guard land together. Original text: add drawing dispatch **inside `locationForPath` only** — `isDrawingPath(path)` → `{ kind: "drawing", path }`, else `{ kind: "file", path }`. Do not branch in `openFile`: it delegates to `replaceTabWithFile` and `navigateToFile`, and `openFileInNewTab` is a separate entry, so drawings would still open as raw text from the sidebar and from wiki-link navigation
...
**Checkpoint**: `locationForPath` is the single path → location constructor and Phase 4 is a no-behavior-change consolidation. Drawing dispatch landed at the head of Phase 5 (T022), together with the page kind, the view, the `ensureFileLoaded` guard, and the five `activeFilePath: path` sites rewritten to derive through `locationPrimaryPath`.
...
- [x] T026 [US1] Add `drawingKind` to the `kinds` tuple in `apps/desktop/src/components/editor-area/page-kinds/index.ts:13` — one entry, nothing else in the registry is touched
- [x] T027 [US1] Create `apps/desktop/src/components/editor-area/drawing-pane.tsx` hosting `<Excalidraw>` behind `React.lazy`, keyed by `location.path`, importing the mandatory `@excalidraw/excalidraw/index.css`
- [x] T028 [US1] Set `window.EXCALIDRAW_ASSET_PATH` to the bundled asset directory and bundle Excalidraw's fonts and workers — the app must work with no network reachable (Principle I, non-negotiable)
- [x] T029 [US1] Mirror `appearance.theme` into Excalidraw's `theme` prop in `apps/desktop/src/components/editor-area/drawing-pane.tsx`. The editor otherwise keeps Excalidraw's own look; restyling it to match Writer is out of scope and would break on every upgrade
- [x] T030 [US1] Debounce `onChange` → save in `apps/desktop/src/components/editor-area/drawing-pane.tsx` at 150ms, matching `SOURCE_CHANGE_DEBOUNCE_MS` in `apps/desktop/src/components/editor-area/mermaid-canvas.ts:31`. Excalidraw's `onChange` fires continuously; an undebounced write would hammer the disk and the watcher
- [x] T031 [US1] On a file that fails to parse, render an explicit error in the tab and **suppress autosave for that tab**. It must not fall back to an empty canvas — the next autosave would overwrite the user's real drawing with nothing (Principle IV; the highest-consequence failure mode in the feature)
- [x] T032 [US1] Register the drawing view in `apps/desktop/src/components/editor-area/page-kinds/views.tsx` — one `drawing` entry pointing at `drawing-pane.tsx`, no footer
- [x] T033 [US1] Confirm the drawing tab's save round-trips through the watcher without a spurious external-change reload (self-write detection in `apps/desktop/src-tauri/src/watcher.rs`). Structurally guaranteed as well as by self-write detection: the `fs:file-changed` handler in `use-file-watcher.ts:36` bails on `!openFiles.get(path)`, and a drawing is never in `openFiles`. Confirm by hand at the checkpoint
...
- [x] T034 [US2] **Branch A only** — verified, no render code needed (checked against `vp build`, not by eye: the entry chunk has no static or dynamic import of Excalidraw's `prod-*` chunk — it only carries the string in a preload manifest — and the `drawings` chunk's only static imports are the paths/tauri helpers, React, and the preload helper, with `loadFromBlob`/`exportToSvg` behind `await import()`): `WIKI_IMAGE_EXTENSIONS` (`apps/desktop/src/lib/wiki-links.ts:124`) already contains `svg`, so `![[x.excalidraw.svg]]` renders through `parseWikiImageEmbedTarget` → `ImageEmbedWidget` (`apps/desktop/src/components/editor-area/wiki-link-extension.ts:141`), and `![](x.excalidraw.svg)` resolves through `image-src-resolver.ts`. Confirm by hand; write no code
...
- [x] T036 [US2] Landed as an editor-level `dblclick` in the existing `wikiLinkClickHandler`, not on the widget: the first click puts the caret inside the token and unfolds it, so the widget node is gone before `dblclick` fires. The target is read on the **first** `mousedown` and stashed, because the drag gate releases on `pointerup` and the unfolded embed collapses from an image to one line of source — by `dblclick` time the coordinates point somewhere else entirely. `dblclick` spends the stash, matched by time (800ms) and position (8px) so a stale press can't open a drawing when the user double-clicks elsewhere. `extractWikiToken` (was `extractWikiTarget`) is now prefix-aware, so a plain `[[x.excalidraw.svg]]` link is left to the click handler instead of navigating twice; the gate is `parseWikiImageEmbedTarget` + `isDrawingPath`, both synchronous, before the event is touched. Resolution goes through the file's existing `resolveEmbed` cache, then `navigateToFile`. Original text: add a `dblclick` handler to the embed widget in `apps/desktop/src/components/editor-area/wiki-link-extension.ts` that resolves the embed target to an absolute path and, when `isDrawingPath` matches, opens it through the editor store (Phase 4's `locationForPath`)
- [x] T037 [US2] No interference, by construction rather than by inspection: the handler returns `false` before touching the event unless the token is an `!`-prefixed embed of a `.excalidraw.svg`, so every other embed keeps today's behaviour untouched. Nothing in the mousedown/click pair can fire on an embed either — `wikiTargetAt` matches `.cm-wiki-link`, while the embed widget is `.cm-image.cm-image-embed` and its unfolded source carries `.cm-wiki-link-editing`. Single click still range-selects the embed. Known cosmetic leftover: the double-click's word selection stays behind on the note being navigated away from
...
- [x] T038 [P] [US3] Landed as `nextAvailableDrawingPath(dir, exists)` — the existence check is a parameter so T039 can exercise the walk without the invoke layer; callers pass `tauri.fileExists`. First free, not highest+1. Original text: add `nextAvailableDrawingPath(dir)` to `apps/desktop/src/lib/drawings.ts`: base name `drawing`, and on collision `drawing-1`, `drawing-2`, … The suffix search is a loop over a file-existence check (`file_exists` in `apps/desktop/src-tauri/src/commands/fs.rs:677`)
- [x] T039 [P] [US3] Add collision-suffix tests to `apps/desktop/tests/drawings.test.ts` with the existence check injected — cover no collision, one collision, a gap in the sequence, and a directory where `drawing-1` exists but `drawing` does not
- [x] T040 [US3] Gated on `root && !isCompactFileMode`, matching `close-tab`/`open-settings`: a standalone compact window has no workspace root and no tab strip. Add a `new-drawing` command to the `Command` list in `apps/desktop/src/components/command-palette/index.tsx` (alongside `new-file` at line 140)
- [x] T041 [US3] The insert hop needed a path → live `EditorView` lookup, since the palette runs outside the editor: `setEditorView`/`clearEditorView`/`insertAtCursor` in `hooks/editor-api.ts`, registered at mount, **re-keyed in the swap effect** (one view is reused across a tab switch, so a mount-only map inserts into the note the pane used to show), and cleared on unmount. The insert is a `view.dispatch`, not a store write: it flows through the update listener into `updateContent` and the save scheduler exactly like a typed character — `updateContent` alone never reaches the view, and `reloadFromDisk` would mark the file clean and drop the embed on the floor. Implement the command in this **load-bearing order**: write the file, **then** insert `![[name.excalidraw.svg]]` into the source document at the cursor, **then** open the drawing tab. Opening the tab moves focus off the note, so an insert afterwards has no cursor to target
- [x] T042 [US3] Free from T022's work: `primaryPath` on the drawing kind stays `null`, so `deriveActiveFilePath` is `null` on a drawing tab, the launcher, and settings alike — `activeFilePath ? getParentDir(activeFilePath) : root` covers all three with no branch per case. When invoked from anywhere that is not a file tab (launcher, settings, another drawing tab), create the file in the workspace root and open the tab, inserting nothing. No error
- [x] T043 [US3] Place the new file in the **note's own directory**. Writer has no attachment-folder concept and this feature does not add one (Principle II — no config key for a value that never varies)
- [~] T044 [US3] **Skipped per its own condition** — it is not one registry entry. Both `folder-context-menu.ts` and `sidebar-surface-context-menu.ts` would need an action id, a handler on the `Handlers` interface, an implementation at each call site, and an update to the spec assertions in `folder-context-menu.test.ts` / `sidebar-surface-context-menu.test.ts`. The command palette covers the flow
