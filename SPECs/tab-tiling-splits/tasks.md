...
- [x] T029 [P] [US4] Add valid v1/v2 and malformed shared JSON fixtures covering missing-before-active migration, duplicate-document history, nested round-trip, missing-file prune/collapse/focus repair, unsupported versions, duplicate/dangling IDs, bad nodes/axes/ratios, cycles, and focused-file prefetch in `SPECs/tab-tiling-splits/fixtures/sessions/`
  - 31 cases plus a `README.md` describing the `{ input, missing_files?, expected: { status, session?, focused_file? } }` shape. A "cycle" is a reused node id, which is the only form JSON can carry. Unsupported page kinds stay a TS-only test: Rust round-trips unknown kinds by design and has no registry to prune against.
- [x] T030 [US4] Add failing TypeScript codec/persistence tests for all shared fixtures, ratio-only and focus persistence, 500 ms coalescing, close flush, restore-disabled behavior, stale workspace callbacks, compact-mode exclusion, diagnostics, and preservation of malformed records in `apps/desktop/tests/session.test.ts` and `apps/desktop/tests/tauri-ipc.test.ts`
  - `tests/session.test.ts` runs every fixture through `parseSession` → `pruneSession` → `focusedSessionFile`, covers encode/decode and the persister (coalescing, ordering, cancel, failure reporting); the workspace-level behaviour (v2 bundle restore with prefetch, malformed hold, layout/focus/ratio-only persistence, restore disabled, compact exclusion, teardown, close flush, stale session read) is in `tests/stores.test.ts` under "workspace-store session persistence".
- [x] T031 [P] [US4] Add failing Rust tests that consume the shared fixtures and require the same validation, migration, pruning, focused-file extraction, null snapshot removal, and locked read/modify/write behavior in `apps/desktop/src-tauri/src/commands/workspace.rs`
  - The codec and its fixture test live in a new `src-tauri/src/session.rs`; `workspace.rs` keeps the commands and tests the modify step (`apply_session_update`) for replace-only-own-key, null removal, and rejection without touching the record. The lock is structural and unchanged.
- [ ] T032 [P] [US4] Add failing pane component tests for focus by body/tab interaction, one globally focused editor among several visible editors, nested resize minima, overflow when viewport is too small, separator keyboard resizing, and pane-last close/launcher fallback in `apps/desktop/tests/editor-pane-layout.test.tsx`
  - **Blocked, not done**, for the reason recorded under T013: the runner is node-only and mounts nothing. What has a pure home is covered: focus-derived routing and pane-last close/launcher fallback in `tests/stores.test.ts`, recursive minima and separator maths in `tests/editor-layout.test.ts`, the committed ratio in the store test "a finished divider drag commits one clamped ratio". Focus by pointer, one focused editor among several visible, keyboard resizing on the separator, and window-too-small overflow are hand checks for T041.
...
- [x] T036 [US4] Implement session v2 codecs, v1 migration, page-kind deserialization, prune/normalize diagnostics, collision-free restored IDs, focused-tab prefetch, and one validated payload API in `apps/desktop/src/lib/session.ts`, `apps/desktop/src/lib/tauri.ts`, and `apps/desktop/src/stores/workspace-store.ts`
  - `lib/session.ts` owns the wire types, `parseSession` (v1 migration inside), `pruneSession`, `focusedSessionFile`, `decodeSession` (page-kind deserialization, re-minted ids, unsupported-kind diagnostics), `encodeSession` (launcher tabs removed, tree collapsed), `createSessionPersister`, and the two IPC wrappers. The store's `restoreSession` takes the decoded `{ tabs, layout }` and publishes it through `publish` like any runtime change; `getEditorSessionSnapshot` is gone.
- [x] T037 [US4] Update `save_session`, `load_session`, and bundled `restore_workspace` to accept/return the versioned payload or null, consume the shared fixtures, preserve `sessions_file_lock`, expose malformed-session diagnostics, and avoid overwriting malformed records before a valid mutation in `apps/desktop/src-tauri/src/commands/workspace.rs`
  - `sessions.json` is now read as `HashMap<String, Value>`: an unreadable file is an error, a malformed entry only affects its own workspace, and `save_session` replaces exactly its key after validating the payload (`AppError::InvalidSession`). `load_session` returns a `status`-tagged `SessionRecord`; a ready record is pruned against the filesystem so the prefetch targets the repaired focused tab, and one pruned to nothing reads as missing.
- [x] T038 [US4] Persist committed layout/navigation/focus/ratio changes from the editor mutation owner after state publication, serialize writes per window, cancel stale callbacks on reset, flush the captured snapshot on explicit close, retain workspace-keyed last-writer-wins behavior, and skip compact windows in `apps/desktop/src/stores/workspace-store.ts` and `apps/desktop/src/lib/session.ts`
  - The editor-store subscription fires on layout or tab identity change (never on document edits), skips the empty transient a reset leaves, and feeds one `SessionPersister` per window (500 ms debounce, chained writes, `cancel` on switch/close). Close schedules and flushes before the close IPC returns and before state is cleared. A malformed record holds its root until a non-empty snapshot for that root is scheduled. Compact windows have no root and never persist.
...
- [x] T039 Add the focused macOS desktop journey for sidebar split, tab move/reorder, post-collapse preview agreement, cancellation, focus routing, resize, and restart restore in `apps/desktop/e2e/specs/tab-tiling.spec.js`
  - `apps/desktop/e2e/specs/tab-tiling.spec.js`. Six cases over the shipped selectors: a sidebar file dropped on a pane's right edge (asserting the held preview rectangle equals the committed pane rectangle, which is the post-collapse agreement contract), a tab moved into the other strip until the source pane collapses, Escape mid-drag leaving layout and overlay untouched, focus following the pane last clicked with exactly one focused pane, keyboard resizing from `[data-pane-separator]`, and the split surviving a reload with its ratio and tabs. Drivable because the drag coordinator is pointer-based with pointer capture, so `performActions` produces a genuine gesture; WebDriver input state persisting between calls is what lets a test hold a drag and read the preview before releasing. **Not yet executed**: `tauri-webdriver` is not installed on this machine and the spec needs a full `--features e2e` app build. Recorded under T040's notes in `quickstart.md`.
- [x] T040 Run `vp check`, `vp test`, `vp run desktop#build`, `cargo test`, `cargo clippy`, and `cargo fmt --check`; record any required test-environment notes in `SPECs/tab-tiling-splits/quickstart.md`
  - All six green: `vp check` (0 errors, 4 pre-existing warnings, none in tiling code), `vp test` (869 tests / 57 files), `vp run desktop#build`, `cargo test` (179 tests, fixtures included), `cargo clippy --all-targets` (0 errors, 9 pre-existing warnings), `cargo fmt --check`. Three environment notes are recorded in `quickstart.md`: `vp` is absent from PATH here so everything ran through `./node_modules/.bin/vp`; `vp check` stops at the first failing stage, so a stray formatting error in an unrelated file hides the lint and type results; and the T039 spec needs `cargo install tauri-webdriver --locked` plus an e2e app build, so it has not been run.
- [ ] T041 Follow the bounded manual matrix in `SPECs/tab-tiling-splits/quickstart.md` for light/dark preview legibility, sidebar move isolation, duplicate Markdown/drawing views, malformed restore diagnostics, compact/Finder regression, and four-note typing/drag/resize traces; record measured document sizes and trace results in that file
  - **Not done, and not doable from here**: every item needs eyes on a running window (light/dark legibility, drag isolation against the file tree, duplicate views, corrupted-record diagnostics, compact/Finder regression, typing and drag/resize traces, a true quit-and-relaunch). The matrix is written out as a checklist under "What is still owed to a human (T041)" in `quickstart.md`; no measurements have been invented to fill it.
- [x] T042 Update the shipped pane, focus, drag, save/reload, and shortcut ownership rules in `docs/editor.md` and `docs/keyboard-shortcuts.md`, then add the user-visible tiling entry to `CHANGELOG.md`
  - `docs/editor.md` ("Panes, tab strips, and drags") and the pane paragraph in `docs/keyboard-shortcuts.md` were already written to the shipped rules during Phase 6 and needed no change. `CHANGELOG.md` did: the two phase-labelled development entries ("phases 4-5", "phase 6", the latter ending in "Still to come" and "Hand verification ... is still owed") were replaced by one user-visible entry in three paragraphs - the drag gestures and what a moved tab keeps, panes as focus-routed editor surfaces with the Ctrl+Tab / Cmd+1-9 rescoping called out, and the layout as part of the session including legacy migration and the malformed-record behaviour.
