...
- [x] T018 [US1] **Validated by the user 2026-09-13** (code complete, `vp build` confirms the vim chunk is lazy: `dist/assets/dist-*.js`, 120 KB, dynamic import only). Manual validation per quickstart.md "Story 1": toggle on → `NORMAL` + block caret; motions/`5j`; `i hello Esc`; `dd u Ctrl+R .`; palette toggle off/on ten times with caret + scroll intact (SC-006); relaunch persists. Also confirm with Vim off that `vp build` puts `@replit/codemirror-vim` in its own chunk not requested at startup (SC-004). Fix anything found before moving on.
...
- [x] T030 [US4] **Validated by the user 2026-09-13.** Manual validation per quickstart.md "Story 4" on `big.md`, including footer labels `VISUAL`, `V-LINE`, `V-BLOCK` and `Esc` dropping the selection
...
- [x] T031 [US5] **Done 2026-09-13: bridge lives in `vim-clipboard.ts` (deps injected, 8 unit tests in `tests/vim-clipboard.test.ts`), acquired/released per view from `vim-mode.ts`; the register is seeded from the clipboard on first enable and an empty register never overwrites it.** Add the clipboard bridge to `apps/desktop/src/components/editor-area/vim-mode.ts` (one module-scoped bridge shared by all views, installed on first enable, torn down when the last view disables): after every `vim-command-done`, read `Vim.getRegisterController().unnamedRegister.toString()`; if it differs from `lastMirrored`, set `lastMirrored` and `writeText` it via `@tauri-apps/plugin-clipboard-manager`. On `window` `focus` and on the editor DOM `copy`/`cut` events, `readText()` and, if the text differs from `lastMirrored`, `unnamedRegister.setText(text)` and update `lastMirrored`. Surface `readText`/`writeText` rejections with `console.error` (do not swallow).
- [x] T032 [US5] **Fixed 2026-09-13: the engine signals `vim-command-done` from `clearInputState`, which runs _before_ the parsed action/operator; the mirror now reads `macroModeState` and the register a microtask later. Turning Vim off mid-recording also ends the recording (without running the library's focus-stealing dialog close).** Confirm the recording indicator path end-to-end: `vim-command-done` in T013 sets `recording` to the register letter (read `macroModeState.latestRegister`) and the footer (T015) shows `recording @a`; stop with `q` clears it. Fix the mirror if the library's state field differs from research R3.
- [x] T033 [US5] **Validated by the user 2026-09-13.** Manual validation per quickstart.md "Story 5" and the coexistence cases: two panes of one note keep independent modes but share registers (FR-008, FR-027); yank → Cmd+V in another app; copy elsewhere → `p`; "toggle Vim off while recording" leaves no stale indicator
...
- [x] T034 [P] Add a "Vim Mode" section to `docs/keyboard-shortcuts.md`: how to enable (Settings / palette), the supported vocabulary by group (motions, operators, text objects, Visual, scrolling, search/substitute, registers/macros/marks), the Ex command table from `contracts/vim-mode.md` §4, the footer indicator, and the notes that Cmd shortcuts keep working in every mode and that motions step over hidden markdown syntax (research R6)
- [x] T035 [P] Add `vim-mode.ts`, `vim-ex-commands.ts`, `vim-store.ts` to the file map in `docs/editor.md` (the `editor-extensions.ts` paragraph) with one line each, plus the rule that `vim()` must stay first in the extension list
- [x] T036 [P] Add a user-facing entry to `CHANGELOG.md` for the completion date: opt-in Vim mode (Settings → Editor → Vim Mode / palette), footer mode indicator and command line, `:w`/`:q`/`:wq`/`:x`, clipboard interop
- [x] T037 **Validated by the user 2026-09-13** (from code: Insert mode binds no `Tab`/`Enter`/`Shift+Tab`, nothing binds Meta; the one deviation, `Esc` leaving Insert and closing a completion together, is recorded in spec Assumptions). Coexistence sweep per quickstart.md "Coexistence and edge cases": Cmd+B/I/K/D/F/Z/Enter and Alt+Arrow in every mode (FR-028); Insert-mode `Enter`/`Tab`/`Shift+Tab`/autocomplete unchanged and `Esc` closes an open completion first (FR-029); `Esc` in Normal closes nothing (FR-030); table cell / mermaid / drawing widgets keep their own keys (FR-004); soft-wrapped paragraph `j`/`k` vs `gj`/`gk`. Record any deviation in `SPECs/vim-mode/spec.md` Assumptions rather than patching the library.
- [x] T038 **SC-004 verified 2026-09-13** (`vp build`: vim is `dist/assets/dist-*.js`, 120.9 KB, one dynamic `import()`, no `modulepreload`); SC-003 typing check validated by the user. Performance check per quickstart.md: Insert-mode typing on `big.md` with Vim on shows no dropped frames vs. off (SC-003); startup with Vim off does not request the vim chunk (SC-004). Investigate and fix any regression before closing.
- [x] T039 Run `vp check && vp test`; move the task in `TODOS.md` to Done; commit per `docs/workflows/agent-loop.md`
