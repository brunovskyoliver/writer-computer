// Writer's default LaTeX snippets. Adapted from Obsidian Latex Suite.
//
// A JavaScript array of objects with `trigger`, `replacement` and `options`
// (plus optional `priority` and `description`). Writer parses this file, it
// never runs it: a function replacement is reported in Settings and skipped.
//
// Option letters: t text - m math - n inline math - M display math -
// A automatic (fires while typing; without A the snippet fires on Tab) -
// r regex trigger - w word boundary.
//
// In a replacement: `$0`..`$9` are tabstops, `${0:x}` a tabstop with a
// placeholder, `[[0]]` the first capture group of a regex trigger,
// `${VISUAL}` the selected text, and `${NAME}` a snippet variable from
// Settings -> LaTeX Suite.
//
// Full reference: docs/latex-suite.md
...
export default [
    ...
    // Insert space after Greek letters and symbols. The negative lookahead
    // means "what is typed so far is not itself a complete symbol", so
    // `\in` + `t` still becomes `\int` while `\alpha` + `x` gets a space.
	{
		trigger: "\\\\(?!(?:${GREEK}|${SYMBOL}|${MORE_SYMBOLS})$)(${GREEK}|${SYMBOL}|${MORE_SYMBOLS})([A-Za-z])",
		replacement: "\\[[0]] [[1]]",
		options: "rmA",
		priority: 2
	},
	...
	// Writer disables automatic snippets while a macro is being typed as an
	// engine rule (a match may not start inside a `\name` word that ends at
	// the caret), so the plugin's priority-3 guard snippet is not needed.
